...
Before committing: run `npm run lint` and `npm run typecheck` (or `npm run check`). Pre-commit runs lint-staged on staged files. See [AGENTS.md](AGENTS.md) and [.cursor/rules](.cursor/rules) for AI-facing standards.
